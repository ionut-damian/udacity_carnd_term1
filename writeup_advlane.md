...
[image1]: ./output_images/project_video1_undist.png "Undistorted"
...
The thresholds are applied to the images in `./advlane.py` (lines 228-244). Here's an example of my output for this step:

![alt text][image3]

#### 3. Describe how (and identify where in your code) you performed a perspective transform and provide an example of a transformed image.

The code for my perspective transform is situated in the function `computePerspectiveTransform()` form the file `calibration.py` (lines 58-63). 
It takes as inputs an image (`img`) and computes a transformation matrix for the perspective transform using the following hardcoded source and destination points:
...
The algorithm is implemented in the function `find_lane_pixels` in `./advlane.py` (lines 100-169). The function requires the image, a starting X position and another image where to draw the visualization as input parameters. Unlike the approach in lesson 7, my function only searches for pixels for a single line. Thus, the starting position for the sliding window has to be passed via parameters. To computer it, I use a histogram over the bottom half of the image to find the X-value with the most line pixels (lines 257, 258, 269 and 281 of the function `process_image`). Thus, the function is called twice, once for the left line, once for the right.
...
The computation of the radius of the curvature is implemented in lines 193 through 207 in my code in `./advlane.py` in the functions `rcurveabs()`. The function is implemented using the equation from lesson 7. The function accepts as parameters a 2nd degree polynomial and a y value.
...
As a variation, the function `rcurve()` ditches the `abs()` to provide a signed radius of the curvature and allow a differentiation of left and right curves.

The vehicle position if computed in `process_image()` line 453 by simply calculating the difference between the lane and image middle points. For the lane middle points I use compute the mean between the left and right lines at the bottom of the image (y = 720).

#### 6. Provide an example image of your result plotted back down onto the road such that the lane area is identified clearly.

I implemented this step in line 447 in my code in `./advlane.py` in the function `process_image()`. The actual warping is handled in the `warp()` function of the `calibration.py` file. to project the lanes back, I use the inverse transformation matrix computed before.
...
The approach performed reasonably well on the `project_video.mp4`, yet had some real difficulties ont eh two challenge videos. To tackle these I implemented multiple checks.

Firstly, a check on the direction in which the sliding window moves within the `find_lane_pixels()` function (described above) could mitigate the chance we jump onto non-line pixels (e.g. barrier, cracks, shadows). This could be done by fitting a polynomial after every window and checking if the next window falls roughly in the path of this polynomial.

Secondly, the algorithm verifies the validity of a polynomial by checking for plausible curvatures (>500 px) for each line. The inverse curvatures of the two lines (lines 310-314) are also compared between eachother. If the difference is too great, both polynomials are ignored.

Thirdly, the distance between to two lines is also used as a sanity check. If the mean distance between the points on the lines is below 3m or above 4.5m, the fitted lines are ignored.

Despite all these checks, the results on the first challenge video are modest at best. The main issue is the shadow area under the bridge which effectively renders the binary masking useless. Despite trying multiple combinations of gradients and color spaces, I have not found any satisfactory solution to the problem.
